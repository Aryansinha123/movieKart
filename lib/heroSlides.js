...
export async function buildHeroSlides(preferredLanguages = [], notInterestedIds = []) {
  ...
  const usedIds = new Set(notInterestedIds.map(Number));
  ...
  // Return slides WITHOUT trailers — the client (HeroSlidesProvider.enrichSlidesWithTrailers)
  // fetches trailers lazily after the carousel is already visible.
  // This prevents TMDB /videos timeout from causing a 502 on the hero endpoint.
  return slides.slice(0, HERO_SLIDE_COUNT);
}
