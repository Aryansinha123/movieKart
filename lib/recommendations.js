...
export async function fetchSimilarMovies(movieIdStr) {
  try {
    ...
    const [p1, p2] = await Promise.all([
      fetch(`${TMDB_BASE}${path}?page=1`, { headers: tmdbHeaders(), next: { revalidate: 86400 } }),
      fetch(`${TMDB_BASE}${path}?page=2`, { headers: tmdbHeaders(), next: { revalidate: 86400 } })
    ]);
...
export async function discoverByGenre(genreId, page = 1) {
  try {
    const res = await fetch(
      ...
      { headers: tmdbHeaders(), next: { revalidate: 86400 } }
    );
    if (!res.ok) return [];
    const data = await res.json();
    return data.results || [];
...
export async function getCuratedCollections() {
  try {
    const [mcuRes, dcRes, sitcomsRes] = await Promise.all([
      fetch(`${TMDB_BASE}/discover/movie?with_companies=420&sort_by=popularity.desc`, { headers: tmdbHeaders(), next: { revalidate: 86400 } }), // Marvel Studios
      fetch(`${TMDB_BASE}/discover/movie?with_companies=429|12806&sort_by=popularity.desc`, { headers: tmdbHeaders(), next: { revalidate: 86400 } }), // DC Films / Entertainment
      fetch(`${TMDB_BASE}/discover/movie?with_genres=35&with_origin_country=US&sort_by=popularity.desc`, { headers: tmdbHeaders(), next: { revalidate: 86400 } }) // American Comedies
    ]);
