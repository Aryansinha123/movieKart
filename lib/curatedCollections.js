...
export async function resolveCollectionMovies(items = []) {
  let pokemonTvCount = 0;
  let powerRangersTvCount = 0;
  const resolved = await Promise.all(
    items.map(async (id, orderIndex) => {
      ...
      const isPokemonTv = id === -60572 || id === -327365 || id === -327367 || id === -220150;
      if (isPokemonTv) {
        if (pokemonTvCount < POKEMON_TV_TITLES.length) {
          title = POKEMON_TV_TITLES[pokemonTvCount];
        }
        pokemonTvCount++;
      }

      const isPowerRangersTv = id === -2328;
      if (isPowerRangersTv) {
        if (powerRangersTvCount < POWER_RANGERS_TV_TITLES.length) {
          title = POWER_RANGERS_TV_TITLES[powerRangersTvCount];
        }
        powerRangersTvCount++;
      }
