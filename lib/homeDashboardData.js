...
import { mapTmdbResult, searchMovies } from "@/lib/tmdb";
...
export async function buildHomeDashboard(userIdStr) {
  ...
  let searchBasedRecommendations = { query: "", movies: [] };
  if (user.recentSearches && user.recentSearches.length > 0) {
    ...
    try {
      ...
      console.error("Failed to load search-based recommendations for dashboard", e);
    }
  }
