...
export default function CollectionPicker({ movieId, className, children }) {
  ...
  async function toggleMovie(collection) {
    ...
    const hasMovie = Array.isArray(collection.movies) && (
      collection.movies.includes(numericMovieId) ||
      collection.movies.includes(String(numericMovieId))
    );
      ...
      <div className="w-full max-w-lg rounded-2xl border border-zinc-800 bg-zinc-950 p-5 shadow-2xl">
        ...
        <div className="mt-4">
          ...
          <div className="mt-2 space-y-2 max-h-[260px] overflow-auto pr-1">
            {isLoading ? (
              ...
              collections.map((c) => {
                const hasMovie = Array.isArray(c.movies) && (
                  c.movies.includes(numericMovieId) ||
                  c.movies.includes(String(numericMovieId))
                );
                return (
                  <button
                    key={c._id}
        ...
        <div className="mt-5 pt-4 border-t border-zinc-800">
          ...
          <button
            ...
            className="mt-4 w-full bg-purple-600 hover:bg-purple-500 disabled:opacity-60 disabled:cursor-not-allowed px-4 py-2.5 rounded-xl font-semibold text-sm transition-colors cursor-pointer text-white"
          >
